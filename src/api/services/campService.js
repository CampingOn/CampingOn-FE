...
export const campService = {
    ...
    // 캠핑장 인기 목록 조회
    getPopularCamps: async (page = 0) => {
        return axios.get(`/api/camps/popular`, {
            params: { page }
        });
    },
    ...
    searchCamps: async (keyword, city, page = 0, size = 12) => {
        return axios.get('/api/camps/search', {
            params: {
                keyword,
                city,
                page,
    ...
    getCampDetail: async (campId) => {
        return axios.get(`/api/camps/${campId}`);
    },

    // 사용자 찜 목록 조회
    getBookmarkedCamps: async (page = 0, size = 3) => {
        return axios.get('/api/camps/bookmarked', {
            params: { page, size }
        });
    }
};